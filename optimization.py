...
def best_reg_logistic(tX_te, tX_tr, y_tr,y_te, degree_max=6, max_iters=150, gamma=0.01):
    deg = np.ones(tX_tr.shape[1],np.int64)
    ...
    for feat_ind in range(len(deg)):
        ...
        for degree_ in degrees:
            ...
            w, loss = reg_logistic_regression(y_tr, tX_tr_tmp,init_w, best_lambda, max_iters, gamma)
            y_pred = predict_labels(w,tX_te_tmp)
            res = np.where(y_te[:,] == y_pred[:,], 1, 0)
            grade = np.mean(res)
            grades.append(grade)
    ...
    for lambda_ in lambdas:
        ...
        w, loss = reg_logistic_regression(y_tr, tX_tr_tmp,init_w, lambda_, max_iters, gamma)
        y_pred = predict_labels(w,tX_te_tmp)

        res = np.where(y_te[:,] == y_pred[:,], 1, 0)
        grade = np.mean(res)
        grades_tmp.append(grade)

    best_lambda= lambdas[np.argmax(grades_tmp)]
            #sigma=sigmoid(tX_te_tmp,w)
            #loss_te = -np.mean(y_te*np.log(sigma)+(1-y_te)*np.log(1-sigma))
            #rmse_te.append(loss_te)
    return deg, best_lambda